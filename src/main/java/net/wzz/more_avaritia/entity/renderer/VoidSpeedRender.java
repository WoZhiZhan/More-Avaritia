...
@OnlyIn(Dist.CLIENT)
public class VoidSpeedRender extends EntityRenderer<VoidSpeedEntity> {
  private static final ResourceLocation VOID = new ResourceLocation("more_avaritia", "textures/entity/void.png");

  public VoidSpeedRender(EntityRendererProvider.Context context) {
    super(context);
  }
  
  @NotNull
  public ResourceLocation getTextureLocation(@NotNull VoidSpeedEntity p_114482_) {
    return VOID;
  }
  
  public void render(@NotNull VoidSpeedEntity ent, float entityYaw, float ticks, @NotNull PoseStack stack, @NotNull MultiBufferSource buf, int packedLightIn) {
    float age = ent.getAge() + ticks;
    Colour colour = getColour(age, 1.0D);
    double scale = VoidSpeedEntity.getVoidScale(age);
  ...
  public static Colour getColour(double age, double a) {
      return VoidRender.getColour(age, a);
  }
}

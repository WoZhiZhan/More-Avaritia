package net.wzz.more_avaritia.client.renderer;
...
public final class BoltRenderer {
    ...
    public static void renderStrand(Matrix4f mat, VertexConsumer vc, List<Vec3> pts, Vec3 camLocal,
                                    float baseWidth, float r, float g, float b, float alpha) {
    ...
    public static void ribbon(Matrix4f mat, VertexConsumer vc, List<Vec3> pts, Vec3 camLocal,
    ...
    /** 正对摄像机的柔和光斑（径向衰减，没有硬边） */
    public static void renderFlare(Matrix4f mat, VertexConsumer vc, Vec3 center, Vec3 camLocal,
    ...
    /**
     * 任意朝向的柔和圆盘。闪电落地的地面辉光要用它（axis 传 (0,1,0)），
     * 光才会"铺在地上"，而不是竖着一块 billboard 插在地里。
     */
    public static void renderDisc(Matrix4f mat, VertexConsumer vc, Vec3 center, Vec3 axis,
                                  float size, float r, float g, float b, float alpha) {
        if (alpha <= 0.003f || size <= 0f) return;
    ...
    /** UV.y 加 2.0 —— shader 靠这个标记切到径向模式 */
    private static void radialQuad(Matrix4f mat, VertexConsumer vc, Vec3 center, Vec3 x, Vec3 y,
    ...
    private static void vert(Matrix4f mat, VertexConsumer vc, Vec3 p, float u, float v,

...
import static net.wzz.more_avaritia.util.InfinityUtils.rl;
        ...
        for (final TextureAtlasSprite sprite : sprites) {
            ...
            for (final BlockElement element : unbaked) {
                for (final Map.Entry<Direction, BlockElementFace> entry : element.faces.entrySet()) {
                    quads.add(FACE_BAKERY.bakeQuad(element.from, element.to, entry.getValue(), sprite, entry.getKey(), new PerspectiveModelState(ImmutableMap.of()), element.rotation, element.shade, rl("dynamic")));
                }
            }
        }
        return quads;
